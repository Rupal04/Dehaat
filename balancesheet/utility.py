...
def create_csv(data):
    try:
        ...
        final_data_rows = [list(e) for e in itertools.zip_longest(
            column1, column2, column3, column4, column5, column6, fillvalue=" ")]
...
def upload_data_to_database(file_obj):
    try:
        ...
        for row in reader:
            if flag:
                # Following three values make one row-data
                particular1 = row[0]
                y_2015 = row[1]
                y_2016 = row[2]
...
def get_data(**kwargs):
    try:
        ...
        if var_name and year:
            ...
            result = "Value of {} in {}: {}".format(var_name, year, balance[year_val])
